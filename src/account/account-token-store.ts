...
export default class AccountTokenStore {
  readOnly: boolean;
  db: sqlite3.Database;

  constructor (readOnly: boolean) {
    this.readOnly = readOnly;
    this.db = new sqlite3.Database("./db/sessions.db", (readOnly ? sqlite3.OPEN_READONLY : (sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE)));
  ...
  set (token: string, uid: string): Promise<string> {
    return new Promise<string>((resolve: (value: string) => void, reject: (value: any) => void) => {
      if (this.readOnly) return reject('READ_ONLY');
      this.db.all("INSERT OR REPLACE INTO tokens VALUES (?, ?, ?)", [token, new Date().getTime() + TWO_WEEKS, uid], (err: Error | null, row: any) => {
        if (err) return reject(err);
        resolve(token);
      });
    });
