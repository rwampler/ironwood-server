...
export default class ModelEventPublisher {
  ...
  async updateViewTarget (accountId: string, viewX: number, viewY: number) {
    if (!this.running) return;
    await this.publisherSocket.send(['SOCKET:VIEW:SAVE', JSON.stringify({ accountId: accountId, viewX: viewX, viewY: viewY })]);
  }
